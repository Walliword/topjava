...
public class JpaMealRepositoryImpl implements MealRepository {
    ...
    public List<Meal> getAll(int userId) {
        return em.createQuery("SELECT m FROM Meal m LEFT JOIN m.user WHERE m.user.id=:userId ORDER BY m.dateTime DESC", Meal.class)
                .setParameter("userId", userId)
                .getResultList();
    }
    ...
    public List<Meal> getBetween(LocalDateTime startDate, LocalDateTime endDate, int userId) {
        return em.createQuery("SELECT m FROM Meal m LEFT JOIN m.user WHERE m.user.id=:userId and m.dateTime between :startDate and :endDate order by m.dateTime desc", Meal.class)
